...
export function TransactionTable({ transactions }) {
  ...
  // **KEY FIX**: Updated useEffect to handle successful deletion
  useEffect(() => {
    if (deleted && deleted.success && !deleteLoading) {
  ...
  // **KEY FIX**: Filter out selected transactions that might have been deleted
  const displayTransactions = useMemo(() => {
    return paginatedTransactions.filter(transaction => 
      transactions.some(t => t.id === transaction.id)
    );
          ...
          <TableHeader>
            <TableRow>
              <TableHead className="w-[50px]">
                <Checkbox
                  checked={
                    selectedIds.length === displayTransactions.length &&
